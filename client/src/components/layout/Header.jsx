  ...
  return (
    <header className="h-16 flex items-center px-5 gap-4 bg-bg-secondary border-b border-border-default flex-shrink-0 relative z-10">
      ...
      <button
        ...
        className="p-2 -ml-2 text-text-secondary hover:text-accent-blue hover:bg-bg-elevated transition-colors flex items-center gap-2"
      >
      ...
      <div className="flex items-center gap-3 flex-shrink-0 ml-auto">
        {/* Demo Mode Badge */}
        {demoActive && (
          <div className="flex items-center gap-1.5 px-2.5 py-1 border border-risk-high/50 bg-risk-high/10">
            <div className="w-1.5 h-1.5 bg-risk-high" />
        ...
        <div className="relative">
          <select
            ...
            className="appearance-none bg-bg-elevated border border-border-default text-text-secondary text-sm px-3 py-1.5 pr-7 focus:outline-none focus:border-border-active cursor-pointer"
          >
            {CITIES.map(c => (
        ...
        {/* Notification Bell */}
        <button className="relative p-2 text-text-secondary hover:text-text-primary hover:bg-bg-elevated transition-colors">
          <Bell className="w-5 h-5" />
          {activeAlerts > 0 && (
            <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-risk-high flex items-center justify-center text-[9px] font-mono text-white font-bold">
        ...
        {/* User Avatar */}
        <div className="w-8 h-8 bg-accent-blue/20 border border-accent-blue/40 flex items-center justify-center text-accent-blue text-xs font-bold font-mono">
          IC
        </div>
      </div>
    </header>
